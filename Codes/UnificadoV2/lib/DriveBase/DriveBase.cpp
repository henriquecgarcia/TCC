...
DriveBase::DriveBase(MotorDriver& leftMotor, MotorDriver& rightMotor, Encoder& leftEncoder, Encoder& rightEncoder)
	: _leftMotor(leftMotor), _rightMotor(rightMotor), _leftEncoder(leftEncoder), _rightEncoder(rightEncoder),
		...
		_headingPid(10.0f, 1.0f, 0.1f, RobotConfig::CONTROL_PERIOD_MS / 1000.0f),
		_headingReferenceRad(0.0f),
		_headingReferenceValid(false) {}
...
void DriveBase::begin() {
	...
	_headingPid.setLimits(-5.0f, 5.0f);
	_headingPid.reset();
	_headingReferenceValid = false;
}

WheelSample DriveBase::update(float targetLeftMps, float targetRightMps, float dt, float imuYawRad) {
	...
	const bool hasStraightCommand = fabsf(targetLeftMps) > 0.001f && fabsf(targetRightMps) > 0.001f;
	const bool sameDirection = hasStraightCommand && (targetLeftMps * targetRightMps > 0.0f);
	...
	int16_t rightPwm = 0;
	if (sameDirection) {
		const float headingError = imuYawRad;
		const float correction = _headingPid.compute(0.0f, headingError);
		targetLeftMps += correction;
		targetRightMps -= correction;
	} else {
		_headingReferenceValid = false;
		_headingPid.reset();
	}
